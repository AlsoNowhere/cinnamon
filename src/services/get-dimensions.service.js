...
*/
  if (
    !forceResolve &&
    (zx_y_angle < 0 ? zx_y_angle * -1 : zx_y_angle) > this.aperture.y
  ) {
    return null;
  }
  if (
    !forceResolve &&
    (x_z_angle < 0 ? x_z_angle * -1 : x_z_angle) > this.aperture.zx
  ) {
    return null;
  }
  ...
  const width = round(halfWidth + (x_z_angle / this.aperture.zx) * halfWidth);
  const height = round(
    halfHeight - (zx_y_angle / this.aperture.y) * halfHeight
  );
